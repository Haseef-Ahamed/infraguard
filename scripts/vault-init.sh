...
echo "Initialising Vault secrets..."
...
SA_TOKEN=$(kubectl create token vault-reviewer -n default --duration=8760h 2>/dev/null \
  || kubectl get secret vault-reviewer-token -n default -o jsonpath='{.data.token}' | base64 -d)
...
echo "Vault init complete. Run: vault kv list infraguard/"
