...
class TestGenerateCluster:
    ...
    def test_output_length(self):
        ...
        assert len(df) == TOTAL_POINTS, (
            f"Expected {TOTAL_POINTS} rows, got {len(df)}"
        )
    ...
    def test_required_columns_present(self):
        ...
        required = [
            "time", "cluster_id",
            "cpu_utilization", "memory_pressure",
    ...
    def test_drift_rate_in_realistic_range(self):
        ...
        assert 0.02 <= rate <= 0.45, (
            f"Drift rate {rate:.2%} is outside realistic range"
        )
    ...
    def test_trig_features_in_minus1_to_1(self):
        df = generate_cluster("test-01", datetime(2026, 1, 1))
